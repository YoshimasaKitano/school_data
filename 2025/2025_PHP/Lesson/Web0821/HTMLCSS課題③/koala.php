<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>OCAサファリパーク in OSAKA</title>
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/style.css">
    <link rel="shortcut icon" href="images/logo50.png">
    <script src="https://kit.fontawesome.com/6643e64c88.js" crossorigin="anonymous"></script>
    <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js"></script>
</head>
<body>
    <?php include(dirname(__FILE__) .'/files/header.html'); ?>
    <main>
        <h2 class="animaltitle">コアラ</h2>
        <h3 class="animalname">～哺乳綱双前歯目コアラ科コアラ属に分類される有袋類～</h3>
        <div class="animalspace">
            <img src="images/koron.png" alt="コアラの写真" class="hipopotamus">
            <p class="animal-text">尾は外部から見えない。オスはメスよりも体重が最大で50%重く、体長も大きい。また、北部の亜種より、南部の亜種の方が25-35%ほど大きい。</p>
        </div>
        <ul class="animalbtn-">
            <li class="animalbtns">
                <a href="bat.html" class="animalbtn"></a>
            </li>
            <li class="animal-btns">
                <a href="peacock.html" class="animal-btn"></a>
            </li>
        </ul>
        <a href="animal.html" class="animaltop">動物たちTOPへ</a>
    </main>
    <?php include(dirname(__FILE__) .'/files/footer.html'); ?>
    <script src="js/main.js"></script>
</body>
</html>