<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>OCAサファリパーク in OSAKA</title>
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/style.css">
    <link rel="shortcut icon" href="images/logo50.png">
    <script src="https://kit.fontawesome.com/6643e64c88.js" crossorigin="anonymous"></script>
    <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js"></script>
</head>
<body>
    <?php include(dirname(__FILE__) .'/files/header.html'); ?>
    <main>
        <h2 class="animaltitle">カバ</h2>
        <h3 class="animalname">～哺乳綱偶蹄目カバ科カバ属に分類される偶蹄類～</h3>
        <div class="animalspace">
            <img src="images/FIONA_HIPPO_FAME_ADV26_5_1616347.jpg" alt="カバの写真" class="hipopotamus">
            <p class="animal-text">陸上動物としてはゾウ、サイに次ぐ3番目の重さとされる。<br>主にサハラ砂漠南のアフリカ大陸の川、湖、沼地などに生息している。</p>
        </div>
        <ul class="animalbtn-">
            <li class="animalbtns">
                <a href="capybara.html" class="animalbtn"></a>
            </li>
            <li class="animal-btns">
                <a href="horse.html" class="animal-btn"></a>
            </li>
        </ul>
        <a href="animal.html" class="animaltop">動物たちTOPへ</a>
    </main>
    <?php include(dirname(__FILE__) .'/files/footer.html'); ?>
    <script src="js/main.js"></script>
</body>
</html>