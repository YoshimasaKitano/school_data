...
def add():
    ...
    if request.method == 'POST':
        ...
        author = request.form['author'].lower().strip()
        count = 0
...
def search():
        ...
        if request.method == 'POST':
            ...
            if keyword == '':
                count += 1
                return render_template('admin/search_result.html', count = count)
                ...
                try:
                    ...
                    emps = cursor.fetchall()
                    return render_template('admin/search_result.html', emps = emps, count = count)
...
def update():
    ...
    if request.method == 'POST':
        title = request.form['title'].lower().strip()
        new_author = request.form['new_author'].lower().strip()
        count = 0

        # 空白で入力された場合の処理
        if title == '' or new_author == '':
            count += 2
            return render_template('admin/update_confirm.html', count = count)
...
# 本の一覧表示する処理
@app.route('/admin_library', methods=['GET'])
def admin_library():
    count = 0
    try:
        conn = get_db_connection()
        ...
        if not emps:
            count += 1
            return render_template('admin/admin_library.html', count = count)

        return render_template('admin/admin_library.html', emps = emps, count = count)
...
def user():
    ...
    if request.method == 'POST':
        ...
        if name == '':
            print('a')
            count += 1
            return render_template('user/user_menu.html', count = count)
        else:
            try:
                conn = get_db_connection()
                print("DB接続成功")
...
# 本の一覧表示する処理
@app.route('/user_library', methods=['GET'])
def user_library():
    count = 0
    try:
        conn = get_db_connection()
        ...
        if not emps:
            count += 1
            return render_template('user/user_library.html', count = count)

        return render_template('user/user_library.html', emps = emps, count = count)
...
def history():
    ...
    try:
        ...
        for book_id in book_ids:
            sql = "SELECT books.title, borrowings.borrowed_at FROM books JOIN borrowings ON books.id = borrowings.book_id WHERE books.id=%s;"
            cursor.execute(sql, (book_id[0], ))
            emps.append(cursor.fetchall())
