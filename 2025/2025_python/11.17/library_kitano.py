"""
図書館管理システム
"""
# モジュール
import os
import json

# ファイルの指定
BOOKS_FILE = "./books.json"
USERS_FILE = "./users.json"

## 各関数の定義
# ユーザーのデータを読み込む
def load_data(file):
    if os.path.exists(file):
        try:
            with open(file, "r", encoding  = "utf-8") as fileobj:
                data = json.load(fileobj) # JSON形式で読み込んで辞書型にする
                return data
        except:
            print("※ データファイルが破損、新しいファイルを作成します。")

    return {}

# ユーザーのデータをファイルに保存する関数
def save_data(file, data):
    try:
        with open(file, "w", encoding = "utf-8") as fileobj:
            # JSON形式でファイルに書き込む(保存)
            json.dump(data, fileobj, indent = 2, ensure_ascii = False)
    except:
        print("※ データの保存に失敗しました。")   

# 本の追加処理を行う関数
def add_book(books):
    while True:
        title = input("タイトルを入力してください。")
        author = input("著者を入力してください。")

        # titleまたはauthorが空白で入力された場合
        if len(title) == 0 or len(author) == 0:
            print("空白で入力しないでください。")   
            continue

        # titleまたはauthorが正常な値で入力された場合
        else:
            break

    # booksの中に値があるかどうかの確認
    if books:

        # 同じタイトルの本の名前がbooksの中にある場合
        if title in books:
            print("すでに登録されています。")

        # 同じタイトルの本の名前が無ければ追加
        else:
            books[title] = {"author": author, "available": True}
            print("追加しました。")

    # 値が無ければそのまま追加する
    else:
        books[title] = {"author": author, "available": True}
        print("追加しました。")

# 本の検索処理を行う関数
def search_books(books):
    while True:
        key = input("キーワードを入力してください。")

        # keyが空白で入力された場合
        if len(key) == 0:
            print("空白で入力しないでください。")   
            continue

        # keyが正常な値で入力された場合
        else:
            break
 
    # カウンターの初期化
    count = 0

    # 読み込んだbooksファイルの中身をループで総当たり(keyに引っかかるものは全て表示するため)
    for book in books:

        # bookにkeyが含まれているか
        if key in book:
            title = book
            author = books[title]["author"]
            available = books[title]["available"]
            print(f"タイトル：{title}\n著者名：{author}\n貸し出し可否：{available}\n")

            # カウンターを使って1つでも含まれているものが見つかったか確認
            count += 1

    # カウンターが一つも増えていなかった＝一つも見つからなかった場合
    if count == 0:
        print("見つかりませんでした。")

# 本の削除処理を行う関数
def delete_book(books):
    while True:
        title = input("タイトルを入力してください。")

        # titleが空白で入力された場合
        if len(title) == 0:
            print("空白で入力しないでください。")   
            continue

        # titleが正常な値で入力された場合
        else:
            break
    
    # カウンターの初期化
    count = 0

    # 削除処理を行いたい本を総当たりで見つける
    for book in books:

        # 削除したい本が見つかった場合
        if title == book:

            # 削除したい本が貸出中の場合
            if books[title]["available"] == False:
                count += 1
                print("現在その本は貸出中です。削除はできません。")
                break
            
            # 削除したい本が貸出中でない場合
            else:
                del books[title]
                count += 1
                print("削除しました。")
                break

    # カウンターが一つも増えていなかった＝一つも見つからなかった場合
    if count == 0:
        print("見つかりませんでした。")

# 本の更新処理を行う関数
def update_book(books):
    while True:
        title = input("タイトルを入力してください。")
        new_author = input("新しい著者名を入力してください。")

        # titleまたはnew_authorが空白で入力された場合
        if len(title) == 0 or len(new_author) == 0:
            print("空白で入力しないでください。")   
            continue

        # titleまたはnew_authorが正常な値で入力された場合
        else:
            break

    # 更新したい本のタイトルが見つかった場合
    if title in books:
        books[title]["author"] = new_author
        print("更新しました。")
    
    # 更新したい本のタイトルが見つかった場合
    else:
        print("見つかりません。")

# 本を借りる処理を行う関数
def borrow_book(books, users, username):

    # ユーザーの本を借りている冊数が3冊の場合
    if len(users[username]["borrowed"]) == 3:
        print("最大3冊までです。返却してください。")

    else:
        while True:
            title = input("タイトルを入力してください。")

            # titleが空白で入力された場合
            if len(title) == 0:
                print("空白で入力しないでください。")   
                continue

            # titleが正常な値で入力された場合
            else:
                break

        # 本が貸出中または本が存在しない場合
        if (title not in books) or (books[title]["available"] == False):
            print("貸出できません。")

        # 本が借りれる状態である時
        else:
            books[title]["available"] = False
            users[username]["borrowed"].append(title)
            print("貸出しました。")
        
# 本の返却処理を行う関数
def return_book(books, users, username):

    # ユーザーの借りている本が1以上の時
    if len(users[username]["borrowed"]) >= 1:
        while True:
            title = input("タイトルを入力してください。")

            # titleが空白で入力された場合
            if len(title) == 0:
                print("空白で入力しないでください。")   
                continue

            # titleが正常な値で入力された場合
            else:
                break

        # カウンターの初期化
        count = 0

        # ユーザーの借りている本を総当たりで確認
        for borrowed in users[username]["borrowed"]:

            # 指定したタイトルが借りている本のタイトルの配列にあった場合
            if title in borrowed:
                books[title]["available"] = True
                users[username]["borrowed"].remove(title)
                count += 1
                print("返却しました。")
                break
        
        # ユーザの借りている本の中に対象の本がなかった場合
        if count == 0:
            print("その本は借りていません。")

    # ユーザーの借りている本の配列の要素数が1未満の時
    else:
        print("借りている本がありません。借りてください。")


# 管理者機能の制御処理をする関数
def admin_menu():
    while True:
        try:
            # 管理者機能の選択
            admin_mode = int(input("@管理者メニュー\n1. 本を追加\n2. 本を検索\n3. 本を削除\n4. 本を更新\n5. 戻る\n選択: [1-5]"))

        except:
            print("数字で入力して下さい。")
            continue

        if admin_mode == 1 or 2 or 3 or 4 or 5:

            if admin_mode == 1:
                add_book(books)

            elif admin_mode == 2:
                search_books(books)

            elif admin_mode == 3:
                delete_book(books)

            elif admin_mode == 4:
                update_book(books)

            # 1でも2でも3でも4でもない場合
            else:
                break

        # 1でも2でも3でも4でも5でもない場合
        else:
            print("無効な選択です。")
            continue

        save_data(BOOKS_FILE, books)

# ユーザー機能の制御処理をする関数
def user_menu(name):
    while True:
        try:
            # ユーザー機能の選択
            user_mode = int(input(f"@ユーザー: [{name}]\n1. 本を借りる\n2. 本を返却する\n3. 貸出履歴を見る\n4. 戻る\n選択: [1-4]"))

        except:
            print("数字で入力して下さい。")
            continue

        if user_mode == 1 or 2 or 3 or 4:

            if user_mode == 1:
                borrow_book(books, users, name)

            elif user_mode == 2:
                return_book(books, users, name)

            elif user_mode == 3:

                # 借りている本がある場合
                if users[name]["borrowed"]:
                    print("現在借りている本は下記の通りです。")
                    for borrowed in users[name]["borrowed"]:
                        print(borrowed, end="\n")

                # 借りている本が無い場合
                else:
                    print("現在借りている本はありません。")

            # 1でも2でも3でもない場合
            else:
                break

        # 1でも2でも3でも4でもない場合
        else:
            print("無効な選択です。")
            continue

        save_data(BOOKS_FILE, books)
        save_data(USERS_FILE, users)

# メイン処理の関数
def main():
    # 管理者モードかユーザーモードかの選択
    while True:
        try:
            mode = int(input("** 図書館システム **\n1. 管理者モード\n2. ユーザーモード\n3. 終了\n選択: [1-3]"))
        except:
            print("数字で入力して下さい。")
            continue
        if mode == 1 or 2 or 3:

            # 管理者モードの場合、管理者メニューを表示する
            if mode == 1:
                admin_menu()

            # ユーザーモードの場合、ユーザーメニューを表示する
            elif mode == 2:

                while True:
                    name = input("あなたの名前を入力してください→")

                    # nameが空白で入力された場合
                    if len(name) == 0:
                        print("空白で入力しないでください。")   
                        continue

                    # nameが正常な値で入力された場合
                    else:
                        break

                # 入力された名前がデータに存在する場合
                if name in users:
                    user_menu(name)

                # 入力された名前がデータに存在しない場合
                else:
                    # 新しいユーザーデータの作成
                    users[name] = {"borrowed": []}
                    save_data(USERS_FILE, users)
                    print("新規ユーザー登録しました。")
                    user_menu(name)

            # 1でも2でもない場合
            else:
                break

        # 1でも2でも3でもない場合
        else:
            print("無効な選択です。")
            continue

# データの読み込み
books = load_data(BOOKS_FILE)
users = load_data(USERS_FILE)

# このファイルがスクリプトとして実行された場合のみmain()を呼び出す
if __name__ == "__main__":
    main()