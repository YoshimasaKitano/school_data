...
def search():
    ...
    try:
        ...
        sql = "SELECT * FROM employee WHERE employee_name like %s;"
        cursor.execute(sql, ("%" + word + "%", ))
        emps = cursor.fetchall()
        return render_template('search.html',emps = emps)
...
def delete(id):
    try:
        ...
        # noからレコードを取得して編集ページへ送る
        sql = "SELECT * FROM employee WHERE employee_no=%s;"
        cursor.execute(sql,(id,))
        emp = cursor.fetchone()
        return render_template('delete_confirm.html', no=emp[0],name=emp[1],password=emp[2])
